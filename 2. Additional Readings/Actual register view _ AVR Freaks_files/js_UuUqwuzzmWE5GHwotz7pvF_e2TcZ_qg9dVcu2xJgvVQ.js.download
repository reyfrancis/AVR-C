...
Drupal.behaviors.quoteHighlighted = {
  attach: function (context, settings) {
    $('a.quote-link:not(.quote-processed)', context)
      ...
      .click(function (e) {
        ...
        if (!textarea || $(textarea).length == 0) {
          var textarea_und = quoteSettings.textarea_und;
          if (!textarea_und || $(textarea_und).length == 0) {
            return true;
          }
          textarea = textarea_und;
        }
        e.preventDefault();
        var quote_post = $(e.target.closest('div.forum-post'));
    ...
    function insertAtCursor(object, text) {
      ...
      else if (Drupal.ckeditorInstance && Drupal.ckeditorInstance) {
        Drupal.ckeditorInstance.insertHtml(text);
      }
      // IE support
    ...
    function getSelectionHtml() {
      ...
      var html = "";
      if (text_selection.rangeCount) {
        var container = document.createElement("div");
        for (var i = 0, len = text_selection.rangeCount; i < len; ++i) {
          children = text_selection.getRangeAt(i).cloneContents().children;
          childNodes = text_selection.getRangeAt(i).cloneContents().childNodes;
          if (childNodes.length == 0) {
            container.appendChild(text_selection.getRangeAt(i).cloneContents());
            continue;
          }
          if (children.length == 0) {
            var p = text_selection.getRangeAt(i).startContainer.parentElement.cloneNode();
            p.append(text_selection.getRangeAt(i).cloneContents());
            if (p.tagName == "LI" || p.tagName == "P") {
              container.appendChild(text_selection.getRangeAt(i).cloneContents());
            }
            else {
          ...
          else if (text_selection.getRangeAt(0).cloneContents().children[0].tagName == 'LI') {
            var p = text_selection.getRangeAt(i).startContainer.parentElement.parentElement.cloneNode();
            p.append(text_selection.getRangeAt(i).cloneContents());
            container.appendChild(p);
          }
          else {
...
;
(function ($) {
  Drupal.behaviors.avrfreaks_forum_select = {
    attach: function (context) {
      jQuery('#edit-taxonomy-forum').change(function() {
        PageIndex = jQuery(this).val();
        if (PageIndex != "none") {
          //location = Drupal.settings.basePath + PageIndex;
          location = PageIndex;
        } 
      });
    }
